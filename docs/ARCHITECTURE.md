...
| `engine/`, `fsx/`, `formats/`, `model/`, `policy/` | Contain no `claude`/`codex`/`hermes` literal in code, and no `switch` on a runtime id. |
...
│   ├── fsx/            safe-read, fingerprint, paths, walk, links
│   ├── formats/        json, jsonc, toml, md+frontmatter, dotenv, starlark, spans
│   ├── model/          kinds, identity, item, schema
│   ├── engine/         environment, scan, normalize, effective, lint, pipeline
...
Exit codes are part of the published contract:
